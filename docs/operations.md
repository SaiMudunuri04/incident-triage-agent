# Incident triage agent operations
...
1. Review the [architecture](architecture.md), tests, image tag, and [Helm values](../k8s/helm/incident-triage-agent/values.yaml).
2. Prepare `RUNBOOK_ROOT`, `METRICS_PATH`, `LLM_BASE_URL`, `LLM_MODEL`, and optionally `LLM_API_KEY` from a secret.
...
4. Render both chart modes locally: `helm lint k8s/helm/incident-triage-agent --strict`, `helm template incident-triage-agent k8s/helm/incident-triage-agent`, and `helm template incident-triage-agent k8s/helm/incident-triage-agent --set autoscaling.enabled=true --set networkPolicy.enabled=true`.

## Verify

- Check `kubectl -n incident-triage-agent rollout status deployment/incident-triage-agent` and inspect `/health/live` and `/health/ready` through the Service.
